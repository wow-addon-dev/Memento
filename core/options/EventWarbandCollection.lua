...
Memento.optionsTable["eventWarbandCollection-mainline"] = {
	...
	args = {
		...
		SEPARATOR_5 = Memento_GetStyleSeparatorText(0.28, " " .. L["options.event.warbandCollection.newHousingItem"]),
		newHousingItemActive = {
			type = "toggle",
			name = L["options.event.general.active.name"]:format(L["options.event.warbandCollection.newHousingItem"]),
			desc = L["options.event.general.active.desc"]:format(L["options.event.warbandCollection.newHousingItem"]),
			get = function()
				return Memento.db.profile.events.warbandCollection.newHousingItem.active
			end,
			set = function(_, value)
				Memento.db.profile.events.warbandCollection.newHousingItem.active = value
			end,
			width = "full",
			order = 0.29
		},
		newHousingItemTimer = {
			name = L["options.event.general.delay.name"],
			desc = L["options.event.general.delay.desc"]:format(L["options.event.warbandCollection.newHousingItem"], 1),
			type = "range",
			min = 0,
			max = 10,
			step = 1,
			disabled = function()
				return not Memento.db.profile.events.warbandCollection.newHousingItem.active
			end,
			get = function()
				return Memento.db.profile.events.warbandCollection.newHousingItem.timer
			end,
			set = function(_, value)
				Memento.db.profile.events.warbandCollection.newHousingItem.timer = value
			end,
			order = 0.30
		},
		LINE_6 = Memento_GetStyleLineNormal(0.31)
	}
}
...
Memento.optionsTable["eventWarbandCollection-tbc"] = {
	...
	args = {
		LINE_1 = Memento_GetStyleLineSmall(0.11),
		SEPARATOR_1 = Memento_GetStyleSeparatorText(0.12, " " .. L["options.event.warbandCollection.newRecipe"]),
		newRecipeActive = {
			type = "toggle",
			name = L["options.event.general.active.name"]:format(L["options.event.warbandCollection.newRecipe"]),
			desc = L["options.event.general.active.desc"]:format(L["options.event.warbandCollection.newRecipe"]),
